     ...
     <!-- 项目列表 -->
            ...
            <div class="pre-list-Sort">
                <div class="pls-top">
                    <div class="pls-top-left">
                        ...
                        <p>已完成合同总额：<span id="Camount"><?php echo $data['Camount_sum'];?></span></p>
                        <p id='not_do'>未完成合同总额：<span id="Pamount"><?php echo $data['not_Camount_sum'];?></span></p>
                        <p>目标总额：<span id="Recei"><?php echo $data['target'];?></span></p>
                        <p>跟踪售前总金额：<span id="shouqian"><?php echo $data['shouqian_sum'];?></span></p>
                    </div>
            ...
            <?php if(is_array($rows)): foreach($rows as $key=>$rows): ?><li id="<?php echo $rows['p_id'];?>" onclick="ProListInfo(this)">
                   ...
                   <div class="pl-con-center">
                    ...
                    <p><a>立项编号：<?php echo $rows['jobcode'];?></a></p>
...
function SubAjaxInfo(){
	...
	$.ajax({  
            url:"/Qh_tpc/index.php/Admin/Project/post_results",  
            type:"post",  
            async:true,  
            ...
            if(data!=''){
				...
				$("#Camount").html(data['Camount_sum']);
          		...
          		$("#Pamount").html(data['not_Camount_sum']);
          		
          		$("#Recei").html(data['target']);
          		
          		$("#shouqian").html(data['shouqian_sum']);
				...
				//$("#Camount").html(0);
          		...
          		var html = "";
          		 ...
          		 for(var i=0;i<rows.length;i++){
 			    	html += '<li id="'+rows[i]['p_id']+'" onclick="ProListInfo(this)"><div class="pl-con-left2"><div class="Focus-on"></div></div><div class="pl-con-center"><p class="p01">'+rows[i]['Jobname']+'</p><p><a>立项：'+rows[i]['Begindate']+'</a></p><p><span class="s02">'+rows[i]['c_name']+'</span></p></div><div class="pl-con-right"></div><div class="pl-con-right2">';
 			    	if(rows[i]['Camount']==0){
 			    	...
 			    	html+='<p><span class="s01"></span><span class="s02"></span><span class="s03">3</span></p> </div></li>';	
...
function ProListInfo(obj){
	...
	//存cookie
	document.cookie="detial_type=yj";
...
$('#time_ok').click(function(){
	...
	var start_year='';
	var stop_year='';
	start_year=start.split("-");
	stop_year=stop.split("-");
		
	if((stop_year[0]-start_year[0])>0){
		$('#not_do').hide();
	}
